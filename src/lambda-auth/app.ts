...
/** Lambda handler */
export const handleAuthInternal = (docClient: DynamoDB.DocumentClient, authTableName: string) => async (
  ...
  let user = 'Unknown';
  try {
    const [u, p] = Buffer.from(authorizationToken.split(' ')[1], 'base64').toString().split(':');
    user = u;
