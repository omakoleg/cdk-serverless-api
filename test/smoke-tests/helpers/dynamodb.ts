...
export const dangerouslyRemoveAuthUser = async (accessId: string): Promise<void> => {
...
export const dangerouslyRemoveUserEvents = async (userId: string): Promise<void> => {
  console.log(`dangerouslyRemoveUserEvents: ${userId}`);
  const events = await loadEventsByUserId(docClient, env.eventsTableName, env.userIdGsiName, userId);
